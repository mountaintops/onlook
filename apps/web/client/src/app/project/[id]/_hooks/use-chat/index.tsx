...
export function useChat({ conversationId, projectId, initialMessages }: UseChatProps) {
    ...
    const { addToolResult, messages, error, stop, setMessages, regenerate, status } =
        useAiChat<ChatMessage>({
            ...
            onFinish: ({ message }) => {
                ...
                if (message.metadata?.chatModel) {
                    const model = message.metadata.chatModel;
                    console.log(`[Response] generated by: ${model.provider} / ${model.model}`);
                }
            },
        });
    ...
    const processedParts = useRef(new Set<string>());
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (finishReason && finishReason !== 'tool-calls') {
            ...
            const applyCommit = async () => {
                ...
                // Generate a descriptive commit title using Mistral
                const commitTitle = await api.git.generateCommitTitle.mutate({ instruction: content });

                // Create checkpoints for all branches with the generated title
                const checkpoints = await createCheckpointsForAllBranches(editorEngine, commitTitle || content);
