...
- **Auto-Fix Missing Icons**: If \`react-icons\` is NOT in \`package.json\`, install it via \`terminal_command\` with \`bun add react-icons\` BEFORE using it.
- **Brand Icons**: Import from \`react-icons/si\` using upper camelCase with \`Si\` prefix (e.g., \`SiDiscord\`).`;
